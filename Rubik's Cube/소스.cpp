...
void CW(int target)
{
	...
	for (int i = 0; i < 3; i++)
	{
		...
		if (tEdge[i]->getDir() != target) tEdge[i]->setDir(color[target].getBy(i + 1));
		//위치를 한 칸 옮긴다
		tEdge[i]->setPos(tEdge[i + 1]->getPos());
	}

	//for문의 행동을 그대로 복사
	if (tEdge[3]->getDir() != target) tEdge[3]->setDir(color[target].getBy(0));
	tEdge[3]->setPos(temp);
	...
	for (int i = 0; i < 3; i++)
	{
		...
		{
			//현재 가리키는 면을 판단하여 다음 면을 가리키게 만든다 color[target].getBy((i + 2) % 4)
			if (tCorner[i]->getDir() == color[target].getBy(i)) tCorner[i]->setDir(color[target].getBy(i + 1));
	...
	//for문의 행동을 그대로 복사  color[target].getBy(0)
	if (tCorner[3]->getDir() != target)
	{
		if (tCorner[3]->getDir() == color[target].getBy(3)) tCorner[3]->setDir(color[target].getBy(0));
		else tCorner[3]->setDir(color[target].getBy(1));
	}
