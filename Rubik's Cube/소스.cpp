...
public:
	...
	void CW(int target)
	{
		...
		{
			//면의 네 엣지 블록의 위치를 지정
			temp = this->color[target].getEdge(i);

			for (int j = 0; j < 12; j++)
			{
				//해당 위치에 존재하는 엣지 블록을 찾았다면
				if (temp == this->edge[j].getPos())
				{
					//리스트에 해당 엣지 블록을 추가
					tEdge[index++] = &(this->edge)[j];
					break;
				}
			}
		...
		{
			//면의 네 코너 블록의 위치를 지정
			temp = this->color[target].getCorner(i);

			for (int j = 0; j < 8; j++)
			{
				//해당 위치에 존재하는 코너 블록을 찾았다면
				if (temp == this->corner[j].getPos())
				{
					//리스트에 해당 코너 블록을 추가
					tCorner[index++] = &(this->corner)[j];
					break;
				}
			}
		...
		for (int i = 0; i < 4; i++)
		{
			...
			if (tEdge[i]->getDir() != target) tEdge[i]->setDir(this->color[target].getBy((i + 1) % 4));
			//위치를 한 칸 옮긴다
			tEdge[i]->setPos(this->color[target].getEdge((i + 1) % 4));
		}
		...
		for (int i = 0; i < 4; i++)
		{
			...
			if (tCorner[i]->getDir() != target)
			{
				...
				if (tCorner[i]->getDir() == this->color[target].getBy(i)) tCorner[i]->setDir(this->color[target].getBy((i + 1) % 4));
				else tCorner[i]->setDir(this->color[target].getBy((i + 2) % 4));
			}
			//위치를 한 칸 옮긴다
			tCorner[i]->setPos(this->color[target].getCorner((i + 1) % 4));
		}
	}
	void ACW(int target)
	{
		...
		for (int i = 0; i < 4; i++)
		{
			...
			if (tEdge[i]->getDir() != target) tEdge[i]->setDir(this->color[target].getBy((i + 3) % 4));
			//위치를 한 칸 옮긴다
			tEdge[i]->setPos(this->color[target].getEdge((i + 3) % 4)); //((i + 3) % 4) 는 ((i - 1) + 4) % 4의 변형
		}
		...
		for (int i = 0; i < 4; i++)
		{
			...
			if (tCorner[i]->getDir() != target)
			{
				...
				if (tCorner[i]->getDir() == this->color[target].getBy(i)) tCorner[i]->setDir(this->color[target].getBy((i + 3) % 4));
				else tCorner[i]->setDir(this->color[target].getBy(i));
			}
			//위치를 한 칸 옮긴다
			tCorner[i]->setPos(this->color[target].getCorner((i + 3) % 4)); //((i + 3) % 4) 는 ((i - 1) + 4) % 4의 변형
		}
	}
};
